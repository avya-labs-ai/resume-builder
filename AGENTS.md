...
- After the self-review, run the mandatory two-stage red-team from `.claude/skills/red-team-review/SKILL.md` (Codex wrapper step 12.6). Stage 1: as a veteran headhunter reading against the JD parse, run exactly five full passes over the English (primary-language) CV and cover letter, editing the `.tex` files in place, then pause for the user's sign-off before any non-primary language (feedback R031). Stage 2: after the non-primary files are generated from the finalized English, verify each is a faithful one-to-one translation and edit it in place. Never fabricate to close a JD gap; reports are chat-only, never saved.
...
- Cover letter: the experience paragraph tells ONE JD-relevant story in depth, never a list of 3+ projects. When the seniority-fit scoring dimension lost points or an obvious red flag exists (overqualification, career pivot, short recent stint), name and defuse the objection in 1-2 sentences — ask the user for the real motivation if unclear; never fabricate one.
