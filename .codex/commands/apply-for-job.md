...
12.6. **Red-team review (mandatory, chat only) — the deeper layer on top of 12.5.** Follow the two-stage red-team spec in `.claude/skills/red-team-review/SKILL.md` (canonical spec), operating on the files in `output/Codex/{slug}/`. **Stage 1 (English / primary):** immediately after 12.5, as a veteran headhunter reading against the step 9.5 JD parse, run **exactly five** full recruiter passes, **editing the primary-language `.tex` files in place** each pass, then print the per-iteration change log and honest residual weaknesses. Never fabricate to close a JD gap; the profile truthfulness boundary and all active `input/feedback.md` rules are hard constraints. After the fifth pass, **pause for the user's explicit sign-off before generating any non-primary language** (enforces feedback R031). **Stage 2 (non-primary):** after sign-off and once each non-primary CV and cover letter are generated from the finalized primary-language documents, verify each is a faithful **one-to-one** translation of the final primary document (nothing added, nothing dropped) and adheres to `lang_rules/{code}.md`; **edit the non-primary `.tex` files in place** to fix any divergence, then print the fidelity report. Both stages' reports are terminal-only.
